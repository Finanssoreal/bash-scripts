...
CONTAINER_IDS=$(docker container ls --all --filter=ancestor=$IMAGE_ID --format "{{.ID}}")
...
docker exec -i "${CONTAINER_IDS[0]}" "mariadb" -u "$DB_USER" --password="$DB_PASS" -e "
DROP DATABASE IF EXISTS $DB_NAME;
CREATE DATABASE IF NOT EXISTS $DB_NAME;"
# import the script
docker exec -i "${CONTAINER_IDS[0]}" "mariadb" -u "${DB_USER}" --password="${DB_PASS}" "${DB_NAME}" < "$DB_FILE"
